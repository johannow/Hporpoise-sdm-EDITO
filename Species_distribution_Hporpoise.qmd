...
#We exclude bathymetry as we only need to load it once, without time information
variables <- rep(names(parameters[1:3]),each=length(monthly_dates))
layer_names <- tibble("month"=rep(monthly_dates,length(parameters)-1))%>%
...
thetao <- terra::rast(raster_list[1:length(monthly_dates)])
so <- terra::rast(raster_list[(length(monthly_dates)+1):(2*length(monthly_dates))])
npp <- terra::rast(raster_list[(2*length(monthly_dates)+1):length(raster_list)])
...
#Without resampling the resolution of the bathymetry is too high
bathy_raster <- resample(terra::rast(new_bathy_url),thetao)
```

```{r}
#An overall prediction accuracy on the test set
